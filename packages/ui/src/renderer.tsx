...
import { InkApp, type InkAppState, type LogLine, type TaskInfo } from './components/InkApp'
...
export class InkRenderer extends BaseRenderer {
  ...
  private handleLogEvent(event: LogEvent): void {
    ...
    updateState({
      logs: [...globalState.logs, logLine].slice(-(this.config.scrollbackLimit || 100)),
    })
  ...
  private handleLoopEnd(event: LoopEndEvent): void {
    ...
    updateState({
      logs: [...globalState.logs, summaryLog].slice(-(this.config.scrollbackLimit || 100)),
    })
  }
  ...
  private handleCliStart(event: CliStartEvent): void {
    ...
    updateState({
      logs: [...globalState.logs, log].slice(-(this.config.scrollbackLimit || 100)),
    })
  }
