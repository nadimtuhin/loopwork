...
export const InkLog: React.FC<InkLogProps> = ({
  ...
  // Get theme colors
  const themeLevelColor = useThemeProp ? useThemeColor(level === 'info' ? 'info' : level as 'success' | 'warning' | 'error' | 'info') : color
  ...
  return (
    <Box>
      {timestamp && <Text color={themeMutedColor}>{timeStr}</Text>}
      {emoji && <Text color={themeLevelColor}>{getEmoji(emoji)} </Text>}
      {prefix && <Text color={themeLevelColor} bold>{prefix} </Text>}
      <Text>{message}</Text>
    </Box>
  )
}
