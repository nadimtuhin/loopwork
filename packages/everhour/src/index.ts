...
export function createEverhourPlugin(config: EverhourConfig = {}): LoopworkPlugin {
  ...
  return {
    ...
    async onTaskStart(context: TaskContext) {
      ...
      const everhourTaskId = getEverhourTaskId(context.task)
      activeTimers.set(context.task.id, { startTime, everhourTaskId })
      ...
      if (everhourTaskId && autoStart) {
        try {
          ...
          console.log(`⏱️ Timer started for ${context.task.id}`)
        } catch (e: any) {
    ...
    async onTaskComplete(context: TaskContext, result: PluginTaskResult) {
      const timerInfo = activeTimers.get(context.task.id)
      activeTimers.delete(context.task.id)
      ...
      if (everhourTaskId && autoStop) {
        try {
          ...
          console.log(`⏱️ Timer stopped for ${context.task.id} (${durationSeconds}s)`)
        } catch (e: any) {
      ...
      if (everhourTaskId && !autoStart) {
        try {
          ...
          console.log(`⏱️ Logged ${durationSeconds}s to ${context.task.id}`)
        } catch (e: any) {
    ...
    async onTaskFailed(context: TaskContext) {
      const timerInfo = activeTimers.get(context.task.id)
      activeTimers.delete(context.task.id)
      ...
      if (timerInfo?.everhourTaskId && autoStop) {
        try {
          await client.stopTimer()
          console.log(`⏱️ Timer stopped for failed task ${context.task.id}`)
        } catch {
