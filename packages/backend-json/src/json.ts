...
import { logger, LoopworkError, DEFAULT_LOCK_TIMEOUT_MS, LOCK_STALE_TIMEOUT_MS, LOCK_RETRY_DELAY_MS } from './utils'
import type {
  TaskBackend,
  Task,
  TaskStatus,
...
import { MarkdownPrdParser } from '@loopwork-ai/spec-parser'
import type { IPrdParser } from '@loopwork-ai/contracts'
...
export class JsonTaskAdapter implements TaskBackend {
  ...
  private parser: IPrdParser

  constructor(config: BackendConfig, parser?: IPrdParser) {
    ...
    this.parser = parser || new MarkdownPrdParser()
  }
  ...
  private async loadFullTask(entry: JsonTaskEntry, data: JsonTasksFile): Promise<Task | null> {
    ...
    try {
      if (fs.existsSync(prdFile)) {
        ...
        try {
          const parsedTask = await this.parser.parse(content)
          ...
          logger.warn(`Failed to parse PRD with parser: ${getErrorMessage(parseError)}`)
        }
