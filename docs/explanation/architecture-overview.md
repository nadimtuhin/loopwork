...
├── packages/checkpoint/            # State persistence & checkpoints
...
interface Task {
  ...
  status: TaskStatus            // pending | in-progress | completed | failed | quarantined | cancelled
  priority: Priority            // high | medium | low | background
  feature?: string              // Feature label for grouping
  parentId?: string             // Parent task (for sub-tasks)
  dependsOn?: string[]          // Task dependencies
  metadata?: Record<string, unknown> // External system IDs and custom data
  url?: string                  // Link to source (GitHub issue, etc.)
  prdFile?: string              // Path to PRD file
  failureCount?: number         // Tracking execution attempts
  lastError?: string            // Last execution error message
  scheduledFor?: string | null  // ISO 8601 schedule
  timestamps?: TaskTimestamps   // Lifecycle tracking
...
interface TaskTimestamps {
  ...
  cancelledAt?: string          // Terminal cancellation
}
...
type TaskEventType = 
  ...
  | 'paused' | 'resumed' | 'quarantined' | 'reset' | 'rescheduled'
  | 'status_change' | 'log' | 'tool_call'
```
...
**Integration:** `src/core/checkpoint-integrator.ts`

The Checkpoint system provides granular state persistence for agents and the core loop, enabling resume capabilities after interruptions or failures.

### Core Components

1.  **CheckpointManager**: Manages the lifecycle of checkpoints (save, restore, events).
2.  **FileCheckpointStorage**: Persists checkpoints as JSON files in `.loopwork/checkpoints/`.
3.  **CheckpointIntegrator**: Connects the main loop and CLI execution to the checkpoint system.

### Checkpoint Structure

```typescript
export interface AgentCheckpoint {
  agentId: string               // Unique ID (e.g., 'loopwork-core' or agent UUID)
  taskId: string                // Current task being executed
  iteration: number             // Current loop/agent iteration
  timestamp: Date               // When checkpoint was created
  phase: string                 // 'started' | 'executing' | 'completed' | etc.
  state: Record<string, unknown> // Custom data (context, memory, stats)
}
```

### Integration Points

- **Parallel Loop**: Creates periodic checkpoints during the execution loop.
- **Task Lifecycle**: Checkpoints created on task start, completion, and failure.
- **CLI Execution**: Captures model selection and execution progress.

### Persistence Strategy

Checkpoints are stored in `.loopwork/checkpoints/{agentId}/context.json`. The `CheckpointManager` implements an in-memory cache with transparent lazy-loading from disk to support state merging across process restarts.
